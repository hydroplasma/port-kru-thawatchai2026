...
BEGIN
    ...
    -- Stats policies
    DROP POLICY IF EXISTS "Public stats are viewable by everyone" ON stats;
    DROP POLICY IF EXISTS "Users can modify stats" ON stats;
    ...
    -- Highlights policies
    DROP POLICY IF EXISTS "Public highlights are viewable by everyone" ON highlights;
    DROP POLICY IF EXISTS "Users can modify highlights" ON highlights;
    ...
    DROP POLICY IF EXISTS "Users can modify works" ON works;
    ...
    DROP POLICY IF EXISTS "Users can modify activities" ON activities;
    ...
    DROP POLICY IF EXISTS "Users can modify certificates" ON certificates;
    ...
    DROP POLICY IF EXISTS "Public pa_categories are viewable by everyone" ON pa_categories;
    DROP POLICY IF EXISTS "Users can modify pa_categories" ON pa_categories;
    ...
    -- PA Indicators policies
    DROP POLICY IF EXISTS "Public pa_indicators are viewable by everyone" ON pa_indicators;
    DROP POLICY IF EXISTS "Users can modify pa_indicators" ON pa_indicators;
    ...
    -- PA Works policies
    DROP POLICY IF EXISTS "Public pa_works are viewable by everyone" ON pa_works;
    DROP POLICY IF EXISTS "Users can modify pa_works" ON pa_works;
    ...
    -- PA Indicator Images policies
    DROP POLICY IF EXISTS "Public pa_indicator_images are viewable by everyone" ON pa_indicator_images;
    DROP POLICY IF EXISTS "Users can modify pa_indicator_images" ON pa_indicator_images;
...
BEGIN
    ...
    -- Works bucket policies
    DROP POLICY IF EXISTS "Public Access Works" ON storage.objects;
    DROP POLICY IF EXISTS "Auth Upload Works" ON storage.objects;
    DROP POLICY IF EXISTS "Auth Delete Works" ON storage.objects;
    ...
    DROP POLICY IF EXISTS "Public Access Activities" ON storage.objects;
    DROP POLICY IF EXISTS "Auth Upload Activities" ON storage.objects;
    DROP POLICY IF EXISTS "Auth Delete Activities" ON storage.objects;
    ...
    -- PA Images bucket policies
    DROP POLICY IF EXISTS "Public Access PA" ON storage.objects;
    DROP POLICY IF EXISTS "Auth Upload PA" ON storage.objects;
    DROP POLICY IF EXISTS "Auth Delete PA" ON storage.objects;
...
WHERE NOT EXISTS (SELECT 1 FROM public.highlights WHERE title = 'ประสบการณ์ 8 ปี');

INSERT INTO public.highlights (title, description, icon_name, color_class, bg_class, display_order)
SELECT 'นักเรียน 300+', 'ดูแลนักเรียนที่ปรึกษาและรายวิชา', 'Users', 'text-green-600', 'bg-green-100', 2
WHERE NOT EXISTS (SELECT 1 FROM public.highlights WHERE title = 'นักเรียน 300+');

INSERT INTO public.highlights (title, description, icon_name, color_class, bg_class, display_order)
SELECT 'พัฒนาตนเอง 100+ ชม.', 'อบรมและพัฒนาวิชาชีพอย่างต่อเนื่อง', 'BookOpen', 'text-purple-600', 'bg-purple-100', 3
WHERE NOT EXISTS (SELECT 1 FROM public.highlights WHERE title = 'พัฒนาตนเอง 100+ ชม.');

INSERT INTO public.highlights (title, description, icon_name, color_class, bg_class, display_order)
SELECT 'รางวัลครูดีเด่น', 'ระดับจังหวัด ปี 2567', 'Award', 'text-orange-600', 'bg-orange-100', 4
WHERE NOT EXISTS (SELECT 1 FROM public.highlights WHERE title = 'รางวัลครูดีเด่น');
...
WHERE NOT EXISTS (SELECT 1 FROM public.stats WHERE title = 'ประสบการณ์สอน');

INSERT INTO public.stats (title, label, icon_name, color_class, display_order)
SELECT 'นักเรียนที่ปรึกษา', '40+', 'Users', 'text-green-500', 2
WHERE NOT EXISTS (SELECT 1 FROM public.stats WHERE title = 'นักเรียนที่ปรึกษา');

INSERT INTO public.stats (title, label, icon_name, color_class, display_order)
SELECT 'ชั่วโมงอบรม', '200+', 'BookOpen', 'text-purple-500', 3
WHERE NOT EXISTS (SELECT 1 FROM public.stats WHERE title = 'ชั่วโมงอบรม');
...
WHERE NOT EXISTS (SELECT 1 FROM public.works WHERE title = 'แผนการจัดการเรียนรู้ Active Learning');
...
WHERE NOT EXISTS (SELECT 1 FROM public.works WHERE title = 'สื่อการสอนออนไลน์ Gen Z');
...
WHERE NOT EXISTS (SELECT 1 FROM public.works WHERE title = 'วิจัยในชั้นเรียนการแก้ปัญหาการอ่าน');
...
WHERE NOT EXISTS (SELECT 1 FROM public.activities WHERE title = 'กิจกรรมวันไหว้ครู 2567');
...
WHERE NOT EXISTS (SELECT 1 FROM public.activities WHERE title = 'เข้าค่ายลูกเสือ-เนตรนารี');
...
WHERE NOT EXISTS (SELECT 1 FROM public.activities WHERE title = 'โครงการโรงเรียนสีขาว');
...
WHERE NOT EXISTS (SELECT 1 FROM public.certificates WHERE title = 'ครูดีเด่นประจำปี 2567');

INSERT INTO public.certificates (title, issuer, year, type, icon_name, color_class, bg_class, display_order)
SELECT 'อบรมการจัดการเรียนรู้ Active Learning', 'Starfish Labz', '2567', 'training', 'Certificate', 'text-blue-600', 'bg-blue-100', 2
WHERE NOT EXISTS (SELECT 1 FROM public.certificates WHERE title = 'อบรมการจัดการเรียนรู้ Active Learning');

INSERT INTO public.certificates (title, issuer, year, type, icon_name, color_class, bg_class, display_order)
SELECT 'วิทยากรโครงการค่ายภาษาไทย', 'โรงเรียนบ้านหนองนา', '2566', 'guest_speaker', 'Mic', 'text-purple-600', 'bg-purple-100', 3
WHERE NOT EXISTS (SELECT 1 FROM public.certificates WHERE title = 'วิทยากรโครงการค่ายภาษาไทย');
